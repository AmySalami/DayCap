...
> "Day Capture" — เก็บภาพแต่ละวันเป็นคลิปสั้นๆ ต่อเนื่อง ติดป้ายเวลา (เวลาจริง HH:MM)
...
เปิดแอป (หน้ากล้อง)
  ...
  → คลิปเก็บ "ในแอป" (ไม่ลงอัลบั้ม) + ติดป้ายเวลา (HH:MM)
...
- ป้าย = **เวลาจริงที่เริ่มถ่าย** รูปแบบ `HH:MM` (ไม่ปัด): `10:01 → 10:01`, `12:58 → 12:58` — **แสดงทับบนวิดีโอเสมอ**
...
>
> **การตัดสินใจ — เผาข้อความด้วย overlay PNG (ไม่ใช้ drawtext):** เรนเดอร์ข้อความ `HH:MM` จาก Flutter เป็น PNG โปร่งใส แล้วให้ ffmpeg ใช้ฟิลเตอร์ `overlay` ซ้อนทับ — เพราะ `drawtext` ต้องการ ffmpeg build ที่มี freetype/fontconfig (ไม่มีในทุก build) แต่ `overlay` มีในทุก build → robust กว่า + คุมฟอนต์/ภาษาไทย/สไตล์ได้เต็มที่จากฝั่ง Flutter
...
class Clip {
  ...
  String label;          // "10:01" (เวลาจริง HH:MM)
  int durationMs;
...
**ฟังก์ชันเวลา:**
```dart
String timeLabel(DateTime dt) => // เวลาจริง HH:MM
...
- [x] ป้ายเวลา: **เวลาจริง `HH:MM`** โชว์แค่เวลา (อัปเดต 2026-07-04 — เดิมปัดชั่วโมง)
